...
local IDSpoofer = {}
IDSpoofer.Config = {
    SpoofUserId      = 438547956,
    SpoofName        = "PandaTM3",
    SpoofDisplayName = "PandaTM3",
    FakeOwnership = {
    },
    Verbose = true,
}
local function log(...)
    if IDSpoofer.Config.Verbose then
        print("[IDSpoofer]", ...)
    end
end
local function tryMeta(obj, key, value)
    local ok = pcall(function()
        local mt = getrawmetatable(obj)
        if not mt then return end
        local old__index    = rawget(mt, "__index")
        local old__newindex = rawget(mt, "__newindex")
        setreadonly(mt, false)
        local prevIndex = old__index
        rawset(mt, "__index", newcclosure(function(self, k)
            if k == key then return value end
            if type(prevIndex) == "function" then
                return prevIndex(self, k)
            elseif type(prevIndex) == "table" then
                return prevIndex[k]
            end
        end))
        local prevNewIndex = old__newindex
        rawset(mt, "__newindex", newcclosure(function(self, k, v)
            if k == key then return end
            if type(prevNewIndex) == "function" then
                prevNewIndex(self, k, v)
            else
                rawset(self, k, v)
            end
        end))
...
local function tryRawset(obj, key, value)
    local ok = pcall(rawset, obj, key, value)
    return ok
end
local function spoof(obj, key, value, label)
    label = label or key
    if tryMeta(obj, key, value) then
        log("Meta hook OK →", label, "=", tostring(value))
        return true
    end
    if tryRawset(obj, key, value) then
        log("Rawset OK →", label, "=", tostring(value))
        return true
    end
    log("FAILED →", label)
    return false
end
function IDSpoofer:SpoofPlayer()
    local lp  = game:GetService("Players").LocalPlayer
    local cfg = self.Config
    if cfg.SpoofUserId then
        spoof(lp, "UserId", cfg.SpoofUserId, "UserId")
    end
    if cfg.SpoofName then
        spoof(lp, "Name", cfg.SpoofName, "Name")
    end
    if cfg.SpoofDisplayName then
        spoof(lp, "DisplayName", cfg.SpoofDisplayName, "DisplayName")
    end
end
function IDSpoofer:SpoofOwnership()
    local MPS = game:GetService("MarketplaceService")
    local cfg = self.Config
    if #cfg.FakeOwnership == 0 then
        log("No fake ownership entries configured — skipping")
        return
    end
    local fakeGamepasses = {}
    local fakeAssets     = {}
    for _, entry in ipairs(cfg.FakeOwnership) do
        if entry.type == "gamepass" then
            fakeGamepasses[entry.id] = true
        elseif entry.type == "asset" or entry.type == "product" then
            fakeAssets[entry.id] = true
        end
    end
    local ok1 = pcall(function()
        local mt = getrawmetatable(MPS)
        setreadonly(mt, false)
        local orig_owns = rawget(mt, "__index")
        local real_UserOwnsGamePassAsync
        real_UserOwnsGamePassAsync = MPS.UserOwnsGamePassAsync
        rawset(mt, "__index", newcclosure(function(self, key)
            if key == "UserOwnsGamePassAsync" then
                return function(s, player, passId)
                    if fakeGamepasses[passId] then
                        log("Faking ownership: gamepass", passId)
                        return true
                    end
                    return real_UserOwnsGamePassAsync(s, player, passId)
                end
            end
            if type(orig_owns) == "function" then
                return orig_owns(self, key)
            elseif type(orig_owns) == "table" then
                return orig_owns[key]
            end
        end))
        setreadonly(mt, true)
    end)
    if ok1 then
        log("Hooked UserOwnsGamePassAsync")
    else
        log("FAILED to hook UserOwnsGamePassAsync")
    end
    local ok2 = pcall(function()
        local mt = getrawmetatable(MPS)
        setreadonly(mt, false)
        local orig_index = rawget(mt, "__index")
        local real_PlayerOwnsAsset = MPS.PlayerOwnsAsset
        local prev = rawget(mt, "__index")
        rawset(mt, "__index", newcclosure(function(self, key)
            if key == "PlayerOwnsAsset" then
                return function(s, player, assetId)
                    if fakeAssets[assetId] then
                        log("Faking ownership: asset", assetId)
                        return true
                    end
                    return real_PlayerOwnsAsset(s, player, assetId)
                end
            end
    ...
    if ok2 then
        log("Hooked PlayerOwnsAsset")
    else
        log("FAILED to hook PlayerOwnsAsset")
    end
end
function IDSpoofer:Verify()
    local lp = game:GetService("Players").LocalPlayer
    log("──── Verify ────")
    log("UserId:     ", lp.UserId)
    log("Name:       ", lp.Name)
    log("DisplayName:", lp.DisplayName)
    log("────────────────")
end
task.defer(function()
    task.wait(1)
    log("Starting...")
    IDSpoofer:SpoofPlayer()
    IDSpoofer:SpoofOwnership()
    IDSpoofer:Verify()
    log("Done.")
end)
return IDSpoofer
